package com.example.lifesaivior.protocol.sync
...
object GcsFilter {
    ...
    fun buildFilter(ids: List<ByteArray>, maxBytes: Int, targetFpr: Double): Params {
        ...
        if (n == 0) {
            return Params(p = p, m = (1L shl p), data = ByteArray(0))
        }
        ...
        val finalM = (trimmedN.toLong() shl p).coerceAtLeast(1L)
    ...
    fun mapToRange(id16: ByteArray, m: Long): Long {
        if (m <= 0L) return 0L
