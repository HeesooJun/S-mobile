...
Lifesaivior는 인터넷/기지국이 없는 재난 상황에서, BLE 중심 오프라인 신호로 피구조자와 구조자를 연결하는 긴급 구조 지원 앱입니다.  
서버 연결이 불안정하거나 끊긴 환경에서도 구조 요청 발신, 탐색, 대응, 통신 흐름을 단말 간 직접 연결로 유지하는 것을 목표로 합니다.

## 서비스 특징
- 다중 트리거 자동 호출: 터치(SOS), 음성, 센서, 무응답 타이머를 결합해 긴급 모드 자동 실행
...
## 주요 기능
1. 통신 고립(인터넷 + 셀룰러 단절) 감지 후 자동 호출 흐름 전환
